...
const HomePage_Balance_Card = ({card_type}:propsType) => {
    ...
    useEffect( ()=>{
        const getdata=async ()=>{
            ...
            if(card_type=="bank"){
                ...
                setlastUpdate( await format_date(info.last_update) )
                setloading(false)
            }
            else if(card_type=="card"){
            ...
            else if(card_type == "savings"){
                setType("Savings")
                const info=( await call_get_saving_acc_balance())
                console.log(info)
                setValue(
                    Number(info.balance_1)
                    +
                    Number(info.balance_2))
                setloading(false)
...
export default HomePage_Balance_Card