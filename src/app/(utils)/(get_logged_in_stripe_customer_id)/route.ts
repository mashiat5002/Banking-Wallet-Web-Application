...
export async function get_stripe_user_id(){
   ...
   if(session){
     ...
     const result =await db.query("Select stripe_customer_id from users where email=? and password=?",[decrypted_session.Email, decrypted_session.Password])
      const res = result[0] as RowDataPacket[];
      
      const id= res[0].stripe_customer_id.replace("https://api-sandbox.dwolla.com/customers/","");
