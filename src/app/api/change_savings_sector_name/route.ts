...
export async function POST( request: NextRequest) {
   ...
   const body= await request.json()
    ...
    try{
      //  const res= await db.query(`Update saving_acc SET ${body.department}="${body.updated_name}" where user_id ="${user_id}"`)
      await connectToDatabase()
      const res=   await saving_acc.updateOne({user_id:user_id},{$set:{[body.department]: body.updated_name}})
      console.log(res)
